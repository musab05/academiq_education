...
import { setCourses, addCourse, setLoading } from '../../store/slices/courseSlice';
import { debounce } from 'lodash';

const Courses = ({ filters: externalFilters = { categories: [], levels: [], ratings: [], price: [] }, onClearFilters, onToggleFilters }) => {
  ...
  const filtered = useMemo(() => {
    ...
    return courses.filter(course => {
      ...
      const matchesCategory = !externalFilters?.categories?.length || 
                             (course.categories && Array.isArray(course.categories) && course.categories.some(cat => 
                               externalFilters.categories.includes(cat._id || cat)
                             ));
      
      const matchesLevel = !externalFilters?.levels?.length || 
                          externalFilters.levels.includes(course.level);
      
      const matchesRating = !externalFilters?.ratings?.length || 
      ...
      const matchesPrice = !externalFilters?.price?.length || 
      ...
      console.log('Course:', course.title, '| Search:', matchesSearch, '| Category:', matchesCategory, '| Level:', matchesLevel, '| Rating:', matchesRating, '| Price:', matchesPrice);
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4 sm:mb-6">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
            ...
            >
              <Plus className="w-4 h-4" /> <span className="hidden sm:inline">Create Course</span><span className="sm:hidden">Create</span>
            </button>
        ...
        {courses.length === 0 ? (
          ...
          <>
            <div className={view === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6' : 'flex flex-col gap-3 sm:gap-4'}>
              {paginatedCourses.map((course) => (
                <CourseCard key={course._id || course.id} course={course} view={view} />
              ))}
            </div>
