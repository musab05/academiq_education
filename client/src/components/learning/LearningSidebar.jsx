...
import { FiPlay, FiCheck, FiDownload, FiLock, FiChevronDown, FiChevronUp, FiPaperclip } from 'react-icons/fi';
import { MdOutlineQuiz } from 'react-icons/md';
import { useParams } from 'react-router-dom';
import api from '../../services/api';
import { useNotification } from '../../context/NotificationContext';

const LearningSidebar = ({ open, lessons, chapters, currentLesson, progress, progressUpdateTrigger, onLessonSelect }) => {
  ...
  useEffect(() => {
    ...
    if (currentLesson?.chapter?._id) {
      setExpandedChapters(prev => ({ ...prev, [currentLesson.chapter._id]: true }));
    }
  }, [currentLesson]);
  ...
  const handleDownloadCertificate = async () => {
    try {
      ...
      link.click();
      link.remove();
      showNotification({ type: 'success', message: 'Certificate downloaded successfully!' });
    } catch (error) {
      showNotification({ type: 'error', message: 'Failed to download certificate' });
    } finally {
  ...
  const calculateOverallProgress = () => {
    ...
    const completedLessons = lessons.filter(lesson => progress[lesson._id]?.status === 'completed').length;
    return Math.round((completedLessons / lessons.length) * 100);
  };

  const calculateChapterProgress = (chapterLessons) => {
    if (!chapterLessons.length) return 0;
    const completedLessons = chapterLessons.filter(lesson => progress[lesson._id]?.status === 'completed').length;
  ...
  chapters.forEach(chapter => {
    const chapterLessons = lessons.filter(lesson => lesson.chapter?._id === chapter._id);
    if (chapterLessons.length > 0) {
      // Use chapter's order field if exists, otherwise use first lesson's order, otherwise use createdAt
      const chapterOrder = chapter.order !== undefined ? chapter.order : 
                          (chapterLessons[0].order !== undefined ? chapterLessons[0].order : 
  ...
  // Sort by order (or createdAt timestamp)
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="p-3 sm:p-4 border-b border-gray-200">
                <div className="flex items-center justify-between mb-2 sm:mb-3">
                  <h3 className="text-sm sm:text-base font-semibold text-gray-900">Course Progress</h3>
                  <button
                    onClick={() => {
                      const event = new CustomEvent('closeSidebar');
                      window.dispatchEvent(event);
              ...
              <div className="flex-1 overflow-y-auto pb-20">
                <div className="p-3 space-y-2">
                  {curriculumItems.map((item, index) => {
                    ...
                    const chapterProgress = calculateChapterProgress(item.lessons);
                      ...
                      >
                        <div className="flex items-center gap-2 flex-1">
                          <div className="w-6 h-6 bg-orange-100 rounded-full flex items-center justify-center flex-shrink-0">
                            <span className="text-xs font-semibold text-orange-600">{chapterIndex + 1}</span>
                          </div>
                          <div className="flex-1 text-left">
                            <h4 className="font-semibold text-gray-900 text-sm">{chapter.title}</h4>
                        ...
                        {isExpanded ? <FiChevronUp className="w-4 h-4 text-gray-500 ml-2" /> : <FiChevronDown className="w-4 h-4 text-gray-500 ml-2" />}
                      </button>
                      ...
                      {isExpanded && (
                        <div className="bg-white">
                          {item.lessons.map((lesson, lessonIndex) => {
                            ...
                            const hasResources = lesson.attachments && lesson.attachments.length > 0;
                  ...
                  {calculateOverallProgress() === 100 ? (
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-3 bg-gray-100 rounded-lg">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium text-gray-900">Certificate Locked</p>
                        <p className="text-xs text-gray-600">Complete all lessons to unlock</p>
                      </div>
                    </div>
                  )}
                </div>
