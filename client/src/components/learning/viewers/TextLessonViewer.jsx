...
const TextLessonViewer = ({
  ...
  const fetchLessonData = async () => {
    try {
      ...
      const progressResponse = await progressAPI.getTextLessonProgress(
        lesson._id,
      );
  ...
  const markAsCompleted = async () => {
    try {
      const currentReadingTime = Math.floor(
        (Date.now() - startTimeRef.current) / 1000,
      );
      console.log(
        "Marking text lesson as completed:",
        lesson._id,
        lesson.title,
      );
      ...
      <div ref={contentRef} className="prose prose-lg max-w-none">
        ...
        {lessonData?.tips && lessonData.tips.length > 0 && (
          <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 my-6">
            <h4 className="text-orange-800 font-semibold mb-2">💡 Tips</h4>
