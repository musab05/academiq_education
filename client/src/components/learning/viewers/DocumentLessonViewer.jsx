    ...
    >
      <div className="mb-8">
        ...
        <div className="flex items-center gap-4 text-sm text-gray-500">
          ...
          <span>{lessonData?.fileType || 'PDF'} • {lessonData?.fileSize || '2.5 MB'} • {lessonData?.pages || '25'} pages</span>
        </div>
      </div>
        ...
        <div className="space-y-4">
          {lessonData?.documents && lessonData.documents.length > 0 ? (
            lessonData.documents
              ...
              .map((document, index) => (
                <div key={document._id || index} className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                  <div className="p-6">
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500">Documents will appear here when added to this lesson.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="prose prose-lg max-w-none">
        <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-6">
          <h3 className="text-lg font-semibold text-blue-800 mb-2">Document Overview</h3>
          <p className="text-blue-700">
            This document contains comprehensive information about the lesson topic. 
            Review it carefully and take notes on key concepts.
          </p>
        </div>

        {lessonData?.viewerSettings && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 my-6">
            <h4 className="text-blue-800 font-semibold mb-2">📄 Document Settings</h4>
            <div className="text-blue-700 text-sm space-y-1">
              <div>Print allowed: {lessonData.viewerSettings.allowPrint ? 'Yes' : 'No'}</div>
              <div>Copy allowed: {lessonData.viewerSettings.allowCopy ? 'Yes' : 'No'}</div>
            </div>
          </div>
        )}
      </div>
