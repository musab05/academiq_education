...
const VideoLessonViewer = ({
  ...
  const fetchLessonData = async () => {
    try {
      ...
      console.log("Video lesson data:", response.data.activity);
      console.log("Video URL:", response.data.activity?.videoUrl);
      console.log("Source type:", response.data.activity?.sourceType);
      ...
      const progressResponse = await progressAPI.getVideoLessonProgress(
        lesson._id,
      );
    ...
    if (!isCompleted) {
      try {
        ...
        const progressPercentage =
          duration > 0
            ? Math.min(100, (effectiveWatchTime / duration) * 100)
            : 0;
    ...
    >
      ...
      <div className="bg-black relative">
        <div className="aspect-video bg-gray-900 flex items-center justify-center relative">
          {lessonData?.videoUrl ? (
            // Check if it's an uploaded video (by URL pattern) or external link
            lessonData.sourceType === "upload" ||
            lessonData.videoUrl.includes("/uploads/") ? (
              <video
                ref={videoRef}
