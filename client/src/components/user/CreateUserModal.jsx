...
const CreateUserModal = ({ isOpen, onClose, onCreateUser, loading, availableRoles, fixedRole = null }) => {
  ...
  // Update role when fixedRole changes
  React.useEffect(() => {
    if (fixedRole) {
      ...
      <div className="bg-white rounded-lg sm:rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 sticky top-0 bg-white">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-900">
            Create {fixedRole ? fixedRole.charAt(0).toUpperCase() + fixedRole.slice(1) : 'User'}
          </h2>
          <button
            onClick={onClose}
