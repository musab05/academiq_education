...
const CreateClassroomModal = ({ onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await classroomManagementAPI.create(payload);
      onSuccess();
    } catch (error) {
      ...
      >
        <div className="p-4 sm:p-6 border-b border-gray-200 sticky top-0 bg-white z-10">
          <div className="flex items-center justify-between">
            <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">Create Live Classroom</h2>
            <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-lg transition-colors tap-target">
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          ...
          <div>
            <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">Classroom Title *</label>
            <input
              type="text"
              required
          ...
          {currentUser?.role === 'superadmin' && (
            <div>
              <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">Institute</label>
              <select
                value={formData.institute}
                onChange={(e) => setFormData({ ...formData, institute: e.target.value, department: '' })}
                className="w-full px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
              >
          ...
          <div>
            ...
            <DepartmentSelector
              ...
              onSelect={(departmentId) => setFormData({ ...formData, department: departmentId })}
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4">
            <p className="text-xs sm:text-sm text-blue-800">
              <strong>Note:</strong> This creates a live-only classroom. After creation, you can schedule live sessions for this classroom.
            </p>
          </div>
