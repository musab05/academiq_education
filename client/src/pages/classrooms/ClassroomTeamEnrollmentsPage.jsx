...
const ClassroomTeamEnrollmentsPage = () => {
  ...
  const fetchClassroom = async () => {
    try {
      ...
      if (found?.enrolledTeams) {
        ...
        const enrolledTeams = teamResponse.data.filter((t) =>
          found.enrolledTeams.includes(t._id),
        );
  ...
  const handleEnroll = async () => {
    ...
    try {
      await classroomManagementAPI.enrollTeam(classroomId, {
        teamId: selectedTeam,
      });
                ...
                <table className="w-full">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      ...
                      <th className="px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase hidden lg:table-cell">
                        Department
                      </th>
                      {canManageClassroom() && (
          ...
          >
            ...
            <TeamSelector
              teams={allTeams.filter(
                (t) => !classroom?.enrolledTeams?.includes(t._id),
              )}
