...
import { Video, Users, Calendar, Plus, Edit, Trash2, ArrowLeft, Clock, BookOpen, Award, TrendingUp } from 'lucide-react';
...
const ClassroomDetailPage = () => {
  ...
  const totalMinutes = studentAttendance.reduce((sum, a) => sum + a.duration, 0);
        ...
        {currentUser?.role !== 'student' ? (
          ...
          <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white px-4 sm:px-6 py-4 sm:py-6">
            ...
            >
              ...
              <span className="text-sm sm:text-base">Back to My Classrooms</span>
            </button>
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold mb-2">{classroom.title}</h1>
            <p className="text-white/90 text-sm sm:text-base">{classroom.description}</p>
          </div>
        )}
        ...
        <div className="flex-1 p-4 sm:p-6 md:p-8">
          <div className="max-w-7xl mx-auto">
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 mb-4 sm:mb-6">
                <div className="flex flex-col lg:flex-row items-start gap-4 lg:justify-between">
                  <div className="flex-1 w-full lg:w-auto">
                    <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 mb-2">{classroom.title}</h1>
                    <p className="text-gray-600 text-sm sm:text-base">{classroom.description}</p>
                    <div className="flex flex-wrap items-center gap-2 sm:gap-4 mt-3 sm:mt-4">
                      <div className="flex items-center gap-2 text-gray-600 text-sm sm:text-base">
                        <Users className="w-4 h-4 sm:w-5 sm:h-5" />
                        <span>{classroom.enrolledStudents?.length || 0} enrolled</span>
                      </div>
                      <div className="flex items-center gap-2 text-gray-600 text-sm sm:text-base">
            ...
            {currentUser?.role === 'student' && (
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6 -mt-3">
                ...
                >
                  ...
                  <p className="text-xl sm:text-2xl font-bold text-gray-900">{attendedSessions}</p>
                ...
                >
                  ...
                  <p className="text-xl sm:text-2xl font-bold text-gray-900">{attendanceRate}%</p>
                ...
                >
                  ...
                  <p className="text-xl sm:text-2xl font-bold text-gray-900">{totalHours}h {remainingMinutes}m</p>
            ...
            {currentUser?.role === 'student' ? (
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-gray-200">
                          <th className="text-left py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm font-semibold text-gray-700">Session</th>
                          <th className="text-left py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm font-semibold text-gray-700 hidden md:table-cell">Date</th>
                        ...
                        {studentAttendance.map((attendance, index) => (
                          ...
                          >
                            <td className="py-3 sm:py-4 px-2 sm:px-4">
                              <p className="font-medium text-gray-900 text-xs sm:text-sm truncate max-w-[150px] sm:max-w-none" title={attendance.sessionTitle}>{attendance.sessionTitle}</p>
                              <p className="text-xs text-gray-500 md:hidden mt-1">
                                {new Date(attendance.sessionDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                              </p>
                            </td>
                            <td className="py-3 sm:py-4 px-2 sm:px-4 text-xs sm:text-sm text-gray-600 hidden md:table-cell">
                              {new Date(attendance.sessionDate).toLocaleDateString('en-US', { 
                                month: 'short', 
                                day: 'numeric', 
                                year: 'numeric' 
                              })}
                            ...
                            <td className="py-3 sm:py-4 px-2 sm:px-4 text-xs sm:text-sm text-gray-600 hidden lg:table-cell">
                              {attendance.attended ? (
                                <span className="font-medium">{Math.floor(attendance.duration / 60)}h {attendance.duration % 60}m</span>
                              ) : (
                            ...
                            <td className="py-3 sm:py-4 px-2 sm:px-4 text-xs sm:text-sm text-gray-600 hidden xl:table-cell">
                              {attendance.joinTime ? (
                                new Date(attendance.joinTime).toLocaleTimeString('en-US', { 
                                  hour: '2-digit', 
                                  minute: '2-digit' 
                                })
                              ) : (
                            ...
                            <td className="py-3 sm:py-4 px-2 sm:px-4 text-xs sm:text-sm text-gray-600 hidden xl:table-cell">
                              {attendance.leaveTime ? (
                                new Date(attendance.leaveTime).toLocaleTimeString('en-US', { 
                                  hour: '2-digit', 
                                  minute: '2-digit' 
                                })
                              ) : (
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
                <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-3 sm:mb-4">Sessions</h2>
                {sessions.length === 0 ? (
                  <div className="text-center py-8 sm:py-12">
                    <Calendar className="w-12 h-12 sm:w-16 sm:h-16 text-gray-300 mx-auto mb-4" />
                    <p className="text-gray-500 text-sm sm:text-base">No sessions scheduled yet</p>
                          ...
                          <div className="flex flex-col sm:flex-row items-start gap-3 sm:justify-between">
                            <div className="flex-1 w-full sm:w-auto min-w-0">
                              <div className="flex items-center gap-2 flex-wrap">
                                <h3 className={`font-semibold text-sm sm:text-base truncate ${sessionEnded ? 'text-gray-500' : 'text-gray-900'}`}>
                                  {session.title}
                                </h3>
                                {sessionEnded && (
                              ...
                              <p className={`text-xs sm:text-sm mt-1 line-clamp-2 ${sessionEnded ? 'text-gray-400' : 'text-gray-600'}`}>
                                {session.description}
                              </p>
                              <div className={`flex flex-wrap items-center gap-2 sm:gap-4 mt-2 text-xs sm:text-sm ${sessionEnded ? 'text-gray-400' : 'text-gray-500'}`}>
                                ...
                                <span className="hidden md:inline">{session.attendance?.filter(a => !a.leaveTime).length || 0} / {session.maxParticipants} participants</span>
                              </div>
                            </div>
