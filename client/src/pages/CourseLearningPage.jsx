...
import { setAllLessonProgress, setLastLesson } from '../store/slices/progressSlice';
...
const CourseLearningPage = () => {
  ...
  const progress = useMemo(() => lessonProgress[courseId] || {}, [lessonProgress, courseId]);
  ...
  useEffect(() => {
    ...
    const handleScormProgressUpdate = () => {
      console.log('SCORM progress update event received, refreshing progress...');
      setTimeout(async () => {
        await refreshProgress();
        setProgressUpdateKey(prev => prev + 1);
      }, 500);
    ...
    return () => {
      ...
      window.removeEventListener('scormProgressUpdate', handleScormProgressUpdate);
    };
  }, [courseId, lessons]);
    ...
    try {
      ...
      for (const lesson of lessons) {
        try {
          ...
          if (lesson.type === 'text') {
            progressResponse = await progressAPI.getTextLessonProgress(lesson._id);
          } else if (lesson.type === 'video') {
            progressResponse = await progressAPI.getVideoLessonProgress(lesson._id);
          } else if (lesson.type === 'block' || lesson.type === 'blocks') {
            progressResponse = await progressAPI.getBlockLessonProgress(lesson._id);
          } else if (lesson.type === 'scorm') {
            progressResponse = await progressAPI.getScormLessonProgress(lesson._id);
            const scormProgress = progressResponse?.data?.cmiData?.['cmi.core.score.raw'] || 
                                 progressResponse?.data?.cmiData?.['cmi.score.raw'] || 0;
            const isScormCompleted = progressResponse?.data?.isCompleted === true;
            ...
            progressResponse = await progressAPI.getAssignmentProgress(lesson._id);
          } else if (lesson.type === 'document') {
            progressResponse = await progressAPI.getDocumentProgress(lesson._id);
          }
          
          if (lesson.type !== 'scorm') {
            const isCompleted = progressResponse?.data?.isCompleted === true;
            console.log(`Lesson ${lesson.title} (${lesson._id}): isCompleted=${isCompleted}`);
          ...
          console.log(`Error fetching progress for lesson ${lesson.title}:`, error.message);
          if (lesson.type !== 'scorm') {
            progressData[lesson._id] = {
              status: 'not_started',
              progress: 0
  ...
  const refreshProgress = async () => {
    if (lessons.length > 0) {
      ...
      console.log('Progress refreshed, new progress:', lessonProgress[courseId]);
    }
  };
  ...
  const fetchCourseData = async () => {
    try {
      ...
      const { lessons: fetchedLessons, chapters: fetchedChapters, course } = response.data;
      ...
      // Add chapters with their lessons
      (fetchedChapters || []).forEach(chapter => {
        const chapterLessons = (fetchedLessons || []).filter(l => l.chapter?._id === chapter._id);
        if (chapterLessons.length > 0) {
          const chapterOrder = chapter.order !== undefined ? chapter.order : new Date(chapter.createdAt).getTime();
          curriculumItems.push({ type: 'chapter', data: chapter, lessons: chapterLessons, order: chapterOrder });
        }
      });
      
      // Add standalone lessons
      (fetchedLessons || []).filter(l => !l.chapter).forEach(lesson => {
        const lessonOrder = lesson.order !== undefined ? lesson.order : new Date(lesson.createdAt).getTime();
        curriculumItems.push({ type: 'lesson', data: lesson, order: lessonOrder });
      });
      ...
      if (orderedLessons.length > 0) {
        ...
        const lastLessonObj = lastLessonId ? orderedLessons.find(l => l._id === lastLessonId) : null;
  ...
  const handleNextLesson = () => {
    ...
    console.log('handleNextLesson - Current lesson:', currentLesson?.title, 'ID:', currentLesson?._id);
    console.log('Current index:', currentIndex, 'Total lessons:', lessons.length);
    console.log('All lesson IDs:', lessons.map(l => ({ id: l._id, title: l.title })));
    console.log('hasNext should be:', currentIndex < lessons.length - 1);
    if (currentIndex !== -1 && currentIndex < lessons.length - 1) {
      const nextLesson = lessons[currentIndex + 1];
      console.log('Moving to next lesson:', nextLesson.title, 'ID:', nextLesson._id);
  ...
  console.log('Render - Current lesson:', currentLesson?.title, 'Index:', currentIndex, 'hasNext:', hasNext, 'hasPrevious:', hasPrevious);
