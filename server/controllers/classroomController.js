...
export const getSessionAnalytics = async (req, res) => {
  try {
    ...
    const activeParticipants = session.attendance.filter(
      (a) => !a.leaveTime,
    ).length;
    const totalDuration = Object.values(userAttendance).reduce(
      (sum, u) => sum + u.totalDuration,
      0,
    );
    const avgDuration =
      uniqueAttendees > 0 ? totalDuration / uniqueAttendees : 0;
    ...
    if (sessionEndTime < now) {
      ...
      if (!canAccessEnded) {
        return res.status(403).json({
          error:
            "This session has ended and is no longer accessible to students.",
          sessionEnded: true,
        });
      }
    }
...
export const createSession = async (req, res) => {
  try {
    ...
    if (currentUser.role === "student") {
      await User.findByIdAndUpdate(currentUser._id, { role: "instructor" });
      roleUpgraded = true;
    }
...
export const deleteSession = async (req, res) => {
  try {
    ...
    res.json({
      message:
        "Session deleted successfully. Attendance records have been preserved for reporting purposes.",
    });
  } catch (error) {
...
export const joinSession = async (req, res) => {
  try {
    ...
    if (session.isPrivate) {
      ...
      const hasValidCode =
        session.accessCode && accessCode === session.accessCode;

      if (!isInstructor && !isAllowed && !hasValidCode) {
        return res
          .status(403)
          .json({ error: "Access denied. This is a private session." });
      }
    }
...
export const uploadResource = async (req, res) => {
  try {
    ...
    if (session.instructor.toString() !== currentUser._id.toString()) {
      return res
        .status(403)
        .json({ error: "Only instructor can upload resources" });
    }
