...
export const createCourse = async (req, res) => {
  try {
    ...
    // Auto-promote user to instructor if they're currently a student
    if (req.user.role === "student") {
      await User.findByIdAndUpdate(req.user._id, { role: "instructor" });
      // Return the new role in response
...
export const getCourseBySlug = async (req, res) => {
  try {
    ...
    console.log(
      "Sending course data with enrollmentCount:",
      enrollmentCount,
      "reviews:",
...
export const addComment = async (req, res) => {
  try {
    const { slug } = req.params;
    const { text } = req.body;
    console.log("Adding comment to course:", slug, "by user:", req.user?._id);
    const course = await Course.findOne({ slug });
    if (!course) {
...
export const addReview = async (req, res) => {
  try {
    ...
    await course.save({ validateBeforeSave: false });
    const savedCourse = await Course.findOne({ slug }).populate(
      "reviews.user",
      "firstName lastName",
    );
    const userReview = savedCourse.reviews.find(
...
export const addFaq = async (req, res) => {
  try {
    ...
    console.log("Adding FAQ to course:", slug, "by user:", req.user?._id);
    const course = await Course.findOne({ slug });
    if (!course) {
      console.log("Course not found:", slug);
      return res.status(404).json({ error: "Course not found" });
...
export const updateCourseBySlug = async (req, res) => {
  try {
    ...
    if (certificateTemplate !== undefined)
      updateData.certificateTemplate = certificateTemplate;
    ...
    if (req.body.allowComments !== undefined)
      updateData.allowComments = req.body.allowComments;
    if (req.body.enrollmentType)
      updateData.enrollmentType = req.body.enrollmentType;
    if (req.body.maxStudents !== undefined)
      updateData.maxStudents = req.body.maxStudents;
    if (req.body.completionCriteria)
      updateData.completionCriteria = req.body.completionCriteria;
    if (req.body.completionPercentage !== undefined)
      updateData.completionPercentage = req.body.completionPercentage;
    if (req.body.emailNotifications !== undefined)
      updateData.emailNotifications = req.body.emailNotifications;
    if (req.body.gradingSystem)
      updateData.gradingSystem = req.body.gradingSystem;
    if (req.body.passingGrade !== undefined)
      updateData.passingGrade = req.body.passingGrade;
    if (req.body.autoEnrollInstituteCourses !== undefined)
      updateData.autoEnrollInstituteCourses =
        req.body.autoEnrollInstituteCourses;
    if (req.body.accessType) updateData.accessType = req.body.accessType;
    if (req.body.requireApproval !== undefined)
      updateData.requireApproval = req.body.requireApproval;
