...
export const getLessonsForCourse = async (req, res) => {
  try {
    ...
    const lessons = await Lesson.find({ course: courseId }).populate('chapter');
...
export const createLesson = async (req, res) => {
  try {
    ...
    if (!title?.trim() || !type || !courseId) {
      return res.status(400).json({ error: 'Title, type, and course ID are required' });
    }
    ...
    const maxLessonOrder = await Lesson.findOne({ course: courseId }).sort('-order').select('order');
    const maxChapterOrder = await Chapter.findOne({ course: courseId }).sort('-order').select('order');
    const maxOrder = Math.max(maxLessonOrder?.order || -1, maxChapterOrder?.order || -1);
...
export const updateLesson = async (req, res) => {
  try {
    ...
    // Allow access for course author, admin, or superadmin
    if (lesson.course.author.toString() !== req.user._id.toString() && 
        req.user.role !== 'admin' && 
        req.user.role !== 'superadmin') {
      return res.status(403).json({ error: 'Access denied' });
    }
...
export const updateChapter = async (req, res) => {
  try {
    ...
    // Allow access for course author, admin, or superadmin
    if (chapter.course.author.toString() !== req.user._id.toString() && 
        req.user.role !== 'admin' && 
        req.user.role !== 'superadmin') {
      return res.status(403).json({ error: 'Access denied' });
    }
