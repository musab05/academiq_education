...
export const updateBlockLesson = async (req, res) => {
  try {
    ...
    processLessonContent(lessonId, lesson.course._id, 'block', updateData).catch(err => 
...
export const updateBlockLessonProgress = async (req, res) => {
  try {
    ...
    let blockProgress = await BlockLessonProgress.findOne({ user: userId, lesson: lessonId });
      ...
      blockProgress = await BlockLessonProgress.findOneAndUpdate(
        { user: userId, lesson: lessonId },
        {
          $set: {
            completedBlocks: completedBlocks || [],
            isCompleted,
            ...(isCompleted && !blockProgress.isCompleted && { completedAt: new Date() })
          }
        },
        { new: true }
      );
...
const updateCourseProgress = async (userId, courseId) => {
  try {
    const blockLessons = await Lesson.find({ course: courseId, type: 'block' }).select('_id');
    ...
    const courseProgress = blockLessons.length > 0 ? Math.round((completedCount / blockLessons.length) * 100) : 0;
    ...
    console.log(`Course ${courseId} block progress: ${courseProgress}% (${completedCount}/${blockLessons.length})`);
  } catch (error) {
