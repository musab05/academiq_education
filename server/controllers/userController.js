...
export const createUser = async (req, res) => {
  try {
    ...
    if (
      ...
      !canCreate[currentUser.role].includes(role)
    ) {
      return res
        .status(403)
        .json({ error: "Access denied to create this role" });
    }
...
export const updateUser = async (req, res) => {
  try {
    ...
    if (
      ...
      !canManage[currentUser.role].includes(user.role)
    ) {
    ...
    if (currentUser.role === "admin" && user.role === "admin") {
      if (
        user.createdBy &&
        user.createdBy._id.toString() !== currentUser._id.toString() &&
...
export const deleteUser = async (req, res) => {
  try {
    ...
    if (
      ...
      !canDelete[currentUser.role].includes(user.role)
    ) {
      return res.status(403).json({ error: "Access denied" });
    }
...
export const getProfileStats = async (req, res) => {
  try {
    ...
    if (userRole === "student") {
      ...
      const query =
        userRole === "admin" && req.user.institute
...
export const resetProfilePicture = async (req, res) => {
  try {
    ...
    // Generate new avatar URL using user's _id
    const generateAvatarUrl = (id) => {
      return `https://api.dicebear.com/8.x/thumbs/svg?seed=${id}`;
    };

    const newProfilePicture = generateAvatarUrl(user._id);
